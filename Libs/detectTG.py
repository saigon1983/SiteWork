...
def detectTG(sku):
    # Функция принимает на вход тип прибора и пытается вернуть ТГ2 и соответствующую ей ТГ1, если ТГ2 однозначно
    # определить не получается, функция запрашивает у пользователя ТГ1.
    # Функция возвращает кортеж (ТГ1, ТГ2)
    sku = sku
    skuType = sku.type.lower()
    TG1 = 'Неопределено'    # Название ТГ1
    TG2 = 'Неопределено'    # Название ТГ2
    def askTG(groupList):
        while True:
